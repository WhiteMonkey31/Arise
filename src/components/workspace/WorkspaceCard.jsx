...
export default function WorkspaceCard({ workspace }) {
  ...
  return (
    <div className="group relative rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-sm hover:shadow-md transition-all duration-200 flex flex-col justify-between min-h-[200px] glow-hover">
      {/* Header */}
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-1.5 flex-1 min-w-0">
          ...
          <Link 
            ...
            className="font-serif font-bold text-base block text-[var(--text)] hover:text-[var(--accent)] transition-colors leading-snug line-clamp-2 mt-2"
          >
            {workspace.name}
          </Link>
        </div>
        ...
        <div className="relative flex-shrink-0">
          ...
          {showMenu && (
            <>
              ...
              <div className="absolute right-0 mt-1 w-36 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-1.5 shadow-lg z-30 flex flex-col gap-0.5 fade-in">
                <Link
                  to={`/workspace/${workspace.id}/overview`}
                  className="rounded-xl px-3 py-2 text-xs font-bold text-[var(--text)] hover:bg-[var(--accent-bg)] block text-left"
                >
                  Open
                </Link>
                <button
                  onClick={handleDuplicate}
                  className="w-full text-left rounded-xl px-3 py-2 text-xs font-bold text-[var(--text)] hover:bg-[var(--accent-bg)] cursor-pointer"
                >
                  Duplicate
                </button>
                <button
      ...
      <div className="flex items-center justify-between border-t border-[var(--border)] pt-4 mt-4">
        <div className="space-y-0.5 leading-none">
          <span className="text-[9px] uppercase font-bold tracking-wider text-[var(--muted)]">Status</span>
        ...
        <div className="flex items-center gap-3">
          <div className="text-right leading-none space-y-0.5">
            <span className="text-[9px] uppercase font-bold tracking-wider text-[var(--muted)]">Budget</span>
