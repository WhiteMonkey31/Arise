...
export default function ScoreBreakdownTable({ scoreBreakdown }) {
  ...
  return (
    <div className="overflow-x-auto border border-(--border) rounded-3xl bg-(--surface) shadow-xs transition-all duration-200">
      <table className="w-full text-xs text-left border-collapse">
        <thead>
          <tr className="border-b border-(--border) bg-(--accent-bg)/20 text-(--muted) font-bold uppercase tracking-wider text-[9px] sm:text-[10px]">
        ...
        <tbody className="divide-y divide-(--border) font-medium text-(--text)">
          {scoreBreakdown.map((row) => {
            ...
            return (
              <tr key={row.name} className="hover:bg-(--accent-bg)/20 transition-colors">
                ...
                <td className="px-5 py-4 text-right">
                  {row.delta === 0 ? (
                    <span className="text-[10px] text-(--muted) font-bold font-sans">On Par</span>
                  ) : (
