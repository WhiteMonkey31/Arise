...
export default function EmptyState({ icon, title, description, actionLabel, onAction }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center p-8 md:p-12 border border-dashed border-(--border] rounded-3xl bg-(--surface] fade-in w-full">
      <div className="h-14 w-14 rounded-2xl bg-(--accent-bg] flex items-center justify-center mb-4 border border-(--border]">
        {getIcon()}
      </div>
      ...
      {actionLabel && onAction && (
        <button
          onClick={onAction}
          className="mt-5 rounded-xl bg-(--accent] px-4 py-2.5 text-xs font-bold text-white shadow-sm hover:opacity-95 transition cursor-pointer"
        >
          {actionLabel}
        </button>
      )}
